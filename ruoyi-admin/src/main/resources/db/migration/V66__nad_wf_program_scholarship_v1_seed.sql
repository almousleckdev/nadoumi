-- Seed workflow definition PROGRAM_WITH_SCHOLARSHIP_V1: 13 stages, 15 transitions,
-- 19 task templates, per docs/WORKFLOW_PROGRAM_WITH_SCHOLARSHIP.md §3-§5, with the
-- DA3 guard substitution (Document/Payment domains don't exist yet — gate on a
-- recorded staff decision instead of the unbacked ALL_REQUIRED_DOCUMENTS_* /
-- PAYMENT_SETTLED predicates). Renumbered from the original spec's V32; also adds
-- the routing columns the original spec assigned to this migration.
-- Rollback (manual): delete from nad_wf_definition where code = 'PROGRAM_WITH_SCHOLARSHIP_V1'
--   (cascades to stage/transition/task_template); alter table nad_program drop column
--   workflow_definition_code; alter table nad_scholarship drop column workflow_definition_code;

alter table nad_program     add column workflow_definition_code varchar(64) null after status;
alter table nad_scholarship add column workflow_definition_code varchar(64) null after status;

insert into nad_wf_definition (code, name, version, status, create_by, create_time)
values ('PROGRAM_WITH_SCHOLARSHIP_V1', 'Program + Scholarship (v1)', 1, 'DRAFT', 'system', now());
...
insert into nad_wf_stage (definition_id, code, name, order_no, stage_type, status_label, sla_hours) values
 ...
 (@def, 'SCHOLARSHIP_DECISION',     'Scholarship decision',         8,  'DECISION', 'DECISION',                 24),
 (@def, 'OFFER_RESPONSE',           'Offer response',               9,  'NORMAL',   'OFFER',                    80),
 (@def, 'PRE_DEPARTURE',            'Pre-departure',                10, 'NORMAL',   'PRE_DEPARTURE',            240),
 (@def, 'ENROLLED',                 'Enrolled',                     11, 'TERMINAL', 'CLOSED_SUCCESS',           null),
 (@def, 'UNSUCCESSFUL',             'Unsuccessful',                 12, 'TERMINAL', 'CLOSED_UNSUCCESSFUL',      null),
 (@def, 'WITHDRAWN',                'Withdrawn',                    13, 'TERMINAL', 'CLOSED_WITHDRAWN',         null);
...
set @st_schdec   := (select id from nad_wf_stage where definition_id = @def and code = 'SCHOLARSHIP_DECISION');
...
insert into nad_wf_transition (definition_id, code, from_stage_id, to_stage_id, guard_json, role_required, auto) values
 (@def, 'submit',              @st_draft,   @st_submit,   '{"all":["FIELD_SET:program_id","FIELD_SET:scholarship_id","FIELD_SET:intake_id"]}', null, 0),
 ...
 (@def, 'uni_offer',            @st_unidec,  @st_schdec,   '{"all":["DECISION_RECORDED:UNIVERSITY_OFFER:OFFER"]}', 'case_officer', 0),
 (@def, 'uni_offer_conditional', @st_unidec, @st_schdec,   '{"all":["DECISION_RECORDED:UNIVERSITY_OFFER:CONDITIONAL_OFFER"]}', 'case_officer', 0),
 ...
 (@def, 'sch_awarded',          @st_schdec,  @st_offer,    '{"all":["DECISION_RECORDED:SCHOLARSHIP_AWARD:AWARDED"]}', 'case_officer', 0),
 (@def, 'sch_declined_body',    @st_schdec,  @st_offer,    '{"all":["DECISION_RECORDED:SCHOLARSHIP_AWARD:REJECT"]}', 'case_officer', 0),
 ...
 -- 'withdraw' is one logical action from 9 different source stages — same code,
 -- distinguished by from_stage_id (uk_wf_transition_definition_code_from); the
 -- engine resolves by (definition, code, current stage), not code alone.
 ...
 (@def, 'withdraw', @st_schdec,  @st_withdrawn, null, null, 0),
...
insert into nad_wf_stage_task_template (stage_id, title, role_required, mandatory, blocks_exit, order_no) values
 ...
 (@st_elig,    'Verify applicant meets scholarship eligibility',      null,               1, 1, 2),
 (@st_elig,    'Record eligibility decision',                        null,               1, 1, 3),
 ...
 (@st_package, 'Complete scholarship application form',                null,               1, 1, 2),
 (@st_package, 'Compile transcripts / certificates package',          null,               1, 1, 3),
 (@st_package, 'Collect application fee',                             'finance',          1, 1, 4),
 (@st_package, 'Internal QA of the package',                          'ops_manager',      1, 1, 5),
 ...
 (@st_offer,   'Explain offer + scholarship outcome to applicant',    null,               1, 0, 1),
