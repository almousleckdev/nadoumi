...
### 5.4 Scholarship (confidentiality split)

| Table | Key columns | Notes / invariants |
| --- | --- | --- |
| `nad_scholarship` | `id`, `title`, `country`, `degree_level`(dict), `field`(dict), `benefits`, `eligibility`, `requirements`, `deadline?`, `binding`(OPEN/PROGRAM_BOUND), `workflow_definition_code?`, `status` | **STUDENT-SAFE columns only.** |
| `v_scholarship_student` *(DB VIEW)* | projection of the safe columns of `nad_scholarship` | The **only** object student/public MyBatis mappers may read. |
| `nad_scholarship_internal` | `scholarship_id` **PK = FK** → `nad_scholarship`, `university_id?`→`nad_university`, `partnership_id?`→`nad_partnership`, `internal_status`, `operational_notes?`, `confidential_terms?` | **CONFIDENTIAL.** 1:1 with `nad_scholarship`. Never joined by student-side queries. |
| `nad_scholarship_program` | `id`, `scholarship_id`, `program_id` | Allowed programmes when `binding='PROGRAM_BOUND'`. **INV5** if `binding='PROGRAM_BOUND'` there is ≥ 1 row; an application's `program_id` must appear here. Note: this table links a scholarship to programmes/universities and is therefore **confidential-adjacent** — it lives on the staff side; discovery uses only `v_scholarship_student`. |
...
| Table | Key columns | Notes |
| --- | --- | --- |
| `nad_contact_inquiry` | `id`, `name`, `email`, `subject?`, `message`, `status`(NEW/READ/RESPONDED/ARCHIVED), `source`(WEBSITE), `locale?`, `ip_address?`, `user_agent?`, `created_at`, `handled_by?`→`sys_user`, `handled_at?` | **Built (V11).** One row per public "Contact us" submission; the row is the source of truth, a failed support-inbox email is logged but not fatal. Written only by the anonymous, IP-rate-limited `POST /api/public/contact`. Honeypot-filtered. Index `(status, created_at)` for the future triage screen. Lives in `nadoumi-identity`; moves to `nadoumi-content`. |
...
| `V12__nad_program.sql` | `nad_program` (`program_type` LANGUAGE/NON_DEGREE/DIPLOMA/BACHELOR/MASTER/PHD), `nad_program_major`, `nad_program_intake`. Programmes belong to a university; exposed only via the university experience. | Step 3 |
| `V13__nad_scholarship.sql` | `nad_scholarship` aggregate + `nad_scholarship_eligibility` / `nad_scholarship_fee*` (typed line items, decimal money) / `nad_scholarship_stipend` / `nad_scholarship_intake*` / `nad_scholarship_document_requirement*` / `nad_scholarship_category`; `nad_scholarship_internal`; view `v_scholarship_student`. | Step 4 |
| `V14__nad_outbox_and_notification.sql` | `nad_outbox_event` + `nad_notification*` + templates. First event: `ScholarshipPublished`. | Step 5 |
| `V15–V16__nad_application*.sql` | `nad_application` (+ `@Version`) + profile/requirement snapshots + append-only children + `nad_wf_*` (one default definition). | Step 6 |
| `V17+__…` | document (+ object storage), communication, payment + finance, employee ops, content, reporting. | Step 7+ |
