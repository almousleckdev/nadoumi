...
| `V15__nad_program.sql` | `nad_program` (`program_type` LANGUAGE/NON_DEGREE/DIPLOMA/BACHELOR/MASTER/PHD), `nad_program_major`, `nad_program_intake`. Programmes belong to a university; exposed only via the university experience. | Step 3 (R2) |
| `V16__nad_scholarship.sql` | `nad_scholarship` aggregate + `nad_scholarship_eligibility` / `nad_scholarship_fee*` (typed line items, decimal money) / `nad_scholarship_stipend` / `nad_scholarship_intake*` / `nad_scholarship_document_requirement*` / `nad_scholarship_category`; `nad_scholarship_internal`; view `v_scholarship_student`. | Step 4 (R3) |
| `V17__nad_outbox_and_notification.sql` | `nad_outbox_event` + `nad_notification*` + templates. First event: `ScholarshipPublished`. | Step 5 |
| `V18–V19__nad_application*.sql` | `nad_application` (+ `@Version`) + profile/requirement snapshots + append-only children + `nad_wf_*` (one default definition). | Step 6 |
