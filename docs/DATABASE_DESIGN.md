...
| `V56__nad_applicant_onboarding_profile.sql` | **DONE (onboarding v2, slice 1).** `nad_applicant` gains `gender`, `country_of_origin`, `country_of_residence`, `native_language`, `wechat_id`, `whatsapp`, `email_verified_at`, `onboarded_at` (+ index on `onboarded_at`). `onboarded_at` is written only by `POST …/onboarding/complete`; existing rows keep it NULL and must finish the new onboarding. Rollback: drop the eight columns. | Applicant ✅ |
| `V57__nad_applicant_passport.sql` | **DONE (onboarding v2, slice 2).** `nad_applicant` gains `passport_media_id` (FK `nad_media_asset`, ON DELETE SET NULL; PROTECTED scan), `passport_given_name`, `passport_family_name`, `passport_dob` (holder data as read from or typed off the passport, compared with the profile), `passport_issue_date`, `passport_expiry_date`, `passport_read_method` (`MRZ`/`MANUAL`), `passport_data_edited`. `passport_no` already existed. Rollback: drop the FK then the eight columns. | Applicant ✅ |
| `V58__nad_applicant_education_detail.sql` | **DONE (onboarding v2, slice 3).** `nad_applicant_education` gains `country`, `city`, `qualification`, `is_current`; `level` is validated on write against `EducationLevel`. | Applicant ✅ |
| `V59__nad_applicant_interest.sql` | **DONE.** `nad_applicant_interest` (one row per applicant: `desired_level`, scholarship interest, intake year/term, teaching language, notes) + `nad_applicant_interest_choice` (`kind` FIELD/CITY, unique per applicant/kind/value, indexed by `(kind, value)` for staff filtering). | Applicant ✅ |
| `V60__nad_applicant_residence.sql` | **DONE.** `nad_applicant_residence` (one row: `in_china`, country, city, address; China only: current education level, school, `visa_type`, `visa_expiry_date`). | Applicant ✅ |
| `V61__nad_applicant_work.sql` | **DONE.** `nad_applicant_work` (repeatable, optional: employer, title, type, country, dates, current, description; China work: `work_visa_type`, `work_visa_expiry`). | Applicant ✅ |
| `V62__nad_applicant_welcomed.sql` | **DONE (slice 4).** `nad_applicant.welcomed_at`: the welcome celebration is shown once, only after `onboarded_at`. | Applicant ✅ |
| `V63__…V89__…` | `nadoumi-application` `nad_wf_*` + `nad_application*` (Step 6); `nadoumi-document` (Step 7); `nadoumi-communication` (conversations/messages); `nadoumi-support` (tickets). Not individually documented in this table yet — see the migration files themselves and their `nadoumi-<module>` module docs. **Payment (Step 9, greenfield)**, content, reporting are still not started. | Step 6+ |
| `V90__nad_university_import_seed_2.sql` | **DONE.** DML-only catalog import #2: 48 more China institutions into `nad_university` (+ 960 `nad_university_highlight`, 30 `nad_university_ranking` child rows), continuing the V54 convention exactly — `status='ACTIVE'`, `publish_status='DRAFT'`, `partner_status='PROSPECT'` (INTERNAL), `create_by='import'`, `reference_code` continues the `NAD-UNI-NNNN` sequence. Combined with V54: 70 imported universities, 1390 highlights, 96 rankings. Every fact was checked via live web search per institution before writing; anything unconfirmed carries an explicit "approximate -- verify" note. Six entries from the original 56-name source list were **dropped, not guessed at**, because the source text had merged two institution names together or named only a city: *SUFT Central South University of Forestry and Technology*; *Guangxi University of Finance and Economics* / *Guangzhou College of Technology and Business* (run together); *Neusoft Institute of Dalian* / *of Guangdong* (run together); *Tongren* (a city, not an institution); *Guangxi Peixian International College*; *Guangxi Talent International College*. Three rows correct a factual assumption in the original request after verification (see each row's `remark`): Wuxi University's real predecessor institution, and the PUBLIC/PRIVATE classification of Nanchang Medical College and Ningbo University of Finance and Economics. Three rows (`Jiangsu Vocational College of Medicine`, `Mianyang Polytechnic`, `Ningbo Polytechnic`) are associate-degree vocational colleges, not bachelor's universities — flagged in `remark` for a staff catalog-inclusion decision, same treatment `Hailing International School` got in V54. Rollback: same as V54 (`delete from nad_university where create_by='import' and reference_code like 'NAD-UNI-%'`; children cascade). Next new migration is **V91**. | University ✅ |

**`FlywayMigrationsIT` counts (after `V62`):** **59 fresh / 58 baselined**
