...
| `V30+__…` | `nad_outbox_event` + `nad_notification*` (Step 5); `nadoumi-application` `nad_wf_*` + `nad_application*` + profile/requirement snapshots (Step 6, **consumes `nadoumi-media`** for JW202/admission attachments); `nadoumi-document` `nad_document*` (Step 7, references `nad_media_asset` via `media_asset_id`); communication, payment + finance, employee ops, content, reporting. Numbers assigned at creation time — **V26–V29 are taken by P1**, so the next new migration is **V30**. | Step 5+ |

**`FlywayMigrationsIT` counts (P1, after `V29`):** **26 fresh / 25 baselined**
migrations (fresh-DB path runs `V1` through `V29`; the baseline-on-migrate path
starts at `V2` since `V1` is the baseline marker). `AbstractNadIntegrationTest`
clears `nad_media_access_log` → `nad_media_asset` (FK order) alongside the
existing table-clear order.
