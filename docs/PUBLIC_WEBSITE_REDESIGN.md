...
| --- | --- | --- | --- |
| **R1 — foundations + shell + static surfaces** | Public design-system layer + honest Home + About + Contact + Universities redesign against **today's** backend. | **B0** contact fields (V14), **B1** university list filters + facets (no migration). | `public/img/` or `@nuxt/image` (D1); rebuild `PageHero` + card family + `SectionHeader` + `Carousel` + `FilterBar`/`FilterDrawer`/`FilterChips` + `ResultGrid` + `Pagination` + `useDiscovery`; redesign `SiteHeader`/`SiteFooter`; **Home** (hero + real "Featured / Popular Universities" carousels + storytelling + How-It-Works `JourneyTimeline` + image `CtaBand`; every not-yet-available section rendered as a labelled "arriving with …" placeholder block, **not** fake cards); **Universities** list (filters/sort/paging/states) + **detail** (hero, section rhythm, gallery slot reserved, Programmes slot reserved); **About** (full IA, real content gaps flagged); **Contact** (info cards + form, all states). |
