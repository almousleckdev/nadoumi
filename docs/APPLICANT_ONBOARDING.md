...
> new profile fields, verified contact email, prefilled applicant at registration. **Slice 2 (protected photo and passport scan, passport details, six-month rule, profile comparison and mismatch message, `PHOTO`/`PASSPORT` completion sections) is IMPLEMENTED, backend and web.** Photo and passport upload are plain file pickers (no in-browser crop/zoom/rotate); the in-browser MRZ OCR reader that shipped with this slice was removed (2026-09-20) as unnecessary complexity — the passport details form is always manual entry, pre-filled with the given name / family name / date of birth already collected on the Personal step so the student is never asked to retype them; only passport number, issue date and expiry date are new. **Slices 3 and 4 (backend and web) are IMPLEMENTED**: the wizard is eight steps —
