...
- **Login flow** (`SysLoginController.login` → `SysLoginService.staffLogin` →
  ...
  0. `LoginUsernameGuard.assertLoginAllowed` — a RuoYi extension point
     ...
     student usernames at this staff-only endpoint. It runs only in
     `staffLogin`, the staff entry point — not in the shared `login` that
     `StudentAuthService`/`StudentEmailOtpController` also call for their own
     (external-user) flows, which must not be subject to this guard.
     `SysLoginController` carries no reference to Nadoumi — the coupling is a
     bean, not an import, keeping RuoYi's generic controller foundation-only
     (CLAUDE.md §3).
...
- **Passport (IMPLEMENTED, onboarding v2 slice 2):** the scan is a PROTECTED `APPLICANT_PASSPORT` media asset, served only through a
  ...
  Passport details are always manual entry (the in-browser MRZ OCR reader was removed 2026-09-20), pre-filled from the profile so the
  student is not asked to retype the given name / family name / date of birth already collected on the Personal step. The server
  compares the confirmed values with the profile, records the entry method (`passport_read_method`, always `MANUAL` from the web
  client today; the column still accepts `MRZ` for contract compatibility) and whether the student edited a pre-filled value
