...
| **3 (R2)** | `nadoumi-program`: program type + majors + intake; Programs shown under the university (admin + public) + Home "Hot programmes" carousels. | V18 |
| **5** | `nad_outbox_event` + poller + `nadoumi-notification` (`nad_notification*` + templates + SSE). First event: `ScholarshipPublished`. | V19 |
