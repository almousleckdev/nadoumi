...
| **5** | `nad_outbox_event` + poller + `nadoumi-notification` (`nad_notification*` + templates + SSE). First events: `ContactInquiryReceived` + `ScholarshipPublished`. **Slices 1–3 + 5 ✅** — `nadoumi-notification` module; `nad_outbox_event` (`V30`) + `OutboxWriter` SPI + `OutboxPollerJob` (`V31`, active); `nad_notification*` model (`V32`/`V33`, `source_ref` idempotency) + `NotificationService` + `/api/notifications` (self) + `/api/staff/notifications` (oversight); `NotificationRenderer` (`{{var}}`, `en` fallback) + `EmailNotificationChannel` (via `MailSender` port) + `NotificationDeliveryDispatcher` + `notificationDispatchJob` (`V34`, active, backoff); `OutboxToNotificationDispatcher` maps events → staff notifications; `ContactService` + `ScholarshipAdminService` emit into the outbox in-transaction (direct support mail removed from `ContactService`). Tests: notification module 17 unit; `NotificationApiTest` + `NotificationPipelineTest` + `OutboxPipelineTest` + `FlywayMigrationsIT` green. **Remaining: slice 4** — SSE `/api/{student,staff}/stream` + Redis pub/sub fan-out (deferred; the admin bell polls `/api/notifications/unread-count` in the interim). | V30–V34 |
| **6** (backend ✅ 2026-09-20; Apply Now / student portal / admin workbench UI still PLANNED) | `nadoumi-application`: `nad_application` + profile/requirement snapshots + append-only children + `nad_wf_*` (one default definition). Public **Apply Now** (self / other student, prefilled, review, submit→snapshot). Student portal: my applications. Admin: application workbench + status change → event → notify. **Consumes `nadoumi-media`** (P1) for JW202 / admission-document attachments via `MediaGateway`. | V63–V68 |
...
| **8b** (backend ✅ 2026-09-20, stacked on Step 8; UI PLANNED) | `nadoumi-support`: help & support tickets. Thin `nad_support_ticket` (status / priority / category / assignment) that owns a 1:1 `SUPPORT` conversation in `nadoumi-communication` (threading, attachments, read status, SSE all reused — no second messaging stack) + append-only `nad_support_ticket_event` audit log. Pool assignment model, no SLA in v1, `CLOSED` is terminal. Additive `ConversationService.openSupport` (applicant-less) and `listSupportMessagesForStaff`. Events `TicketOpened` / `TicketAssigned` / `TicketStatusChanged` via the outbox (`docs/DOMAIN_EVENTS.md`); `OutboxToNotificationDispatcher` gained an optional `audiencePermission` payload override. Spec: `docs/superpowers/specs/2026-09-20-support-ticketing-domain-design.md`. | V87–V89 |
...
| **Email standardization ✅ (2026-09-10, user request)** | One shared, non-boxed Nadoumi email design across **all** transactional email. New `EmailContent` / `EmailLayout` / `EmailRender` / `BrandProperties` in `com.nadoumi.identity.service.mail`; `EmailMessage` gains `htmlBody`; `SmtpMailSender` sends `multipart/alternative`. Identity emails (`otp-*`, `account-exists`) re-rendered through the shell; new `password-changed` security email (`PasswordChangedEvent` + `AFTER_COMMIT` listener). Notification emails wrapped by `EmailNotificationChannel` (SPI `send` gains `type`) with per-type CTAs. **Welcome email:** `StudentRegistered` outbox event → `WelcomeContentComposer` (in `nadoumi-notification`, +read-only deps on `nadoumi-scholarship`/`nadoumi-program`) → `WELCOME` notification (IN_APP) + personalised email (live catalog sections, empty ones dropped). `NotificationType.WELCOME` / `APPLICATION_SUBMITTED` / `APPLICATION_STATUS_CHANGED` + `en` templates seeded; application types now have a producer: `nadoumi-application` (Step 6 backend, 2026-09-20). `nadoumi.web.baseUrl` + `nadoumi.brand.*` config; `/email/**` static assets (placeholder logo + social icons) served anonymously. Docs: `COMMUNICATION_AND_NOTIFICATIONS.md` §4.3–4.4, `DOMAIN_EVENTS.md`, `DEVELOPMENT_GUIDELINES.md` §6a. Tests: identity 67, notification 29 green (DB integration tests need Docker). Spec: `docs/superpowers/specs/2026-09-10-email-templates-standardization-design.md`. | V55 |
| **Admin console build-out ✅ (2026-09-06, pulled forward at user request)** | `nadoumi-admin` (custom Vue 3 + Element Plus console) filled out against the **existing** RuoYi `/system/*` + `/monitor/*` backend — no new backend module. Delivered: full **My profile** (avatar / details / password); **Staff** + **Students** (one component, `user_type` route param — additive `userType` on RuoYi `SysUser` + `SysUserMapper`); **Roles** (menu-tree + data scope); **Menus & Permissions**; **Departments**; **Posts**; **Dictionaries**; **Configuration**; **Scheduled Jobs** (run/pause/CRUD); **Operation Logs** + **Sign-in Logs**; **Notifications** oversight (`/api/staff/notifications`) + navbar bell (`/api/notifications/unread-count`). `nav.ts` + router + `en` i18n wired; `vue-tsc` + 74 Vitest + eslint + production build green. **Still PLANNED:** Applications/Documents (Step 6/7), Partnerships, **Finance/Payments/Payroll (greenfield — Step 9, no backend)**, CMS, Conversations, Reports, online-users/cache/server monitor. | — (RuoYi-backed) |
...
**Migration numbering (updated after Step 5 slices 1–3/5):** `V26`–`V29` applied
(media layer, §P1); `V30`–`V34` applied (outbox + notification model + seeds +
dispatch job, Step 5). Migration numbers are assigned at creation time — the
**next new migration is `V35`** (Step 5 SSE slice, or Step 6). `docs/DATABASE_DESIGN.md`
§6 carries the authoritative ledger.
