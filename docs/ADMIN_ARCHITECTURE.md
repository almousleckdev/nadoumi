...
| Scholarships — list + create/edit + detail | **IMPLEMENTED** | `nadoumi-scholarship` module (V16 / V17 + `hero_image_url` / `cover_image_url` V21). Sectioned drawer (Identity / Classification / Intakes / Eligibility / Fees / Stipend / Documents / Content / **Media** / Publication); slug is auto-derived (no field); category options filter by funding model (SELF none; PARTIAL excludes CSC/CGS/Type A–D). Hero + cover images uploaded from disk (`ImageUpload`). Detail page + confidential university/partnership linkage sub-form behind `nad:scholarship:internal:view` / `:edit`. `nad:scholarship:list/view/create/edit/remove/publish`. |
